...
@section('page_title', 'Dashboard Cutting')

@section('content')
<div class="d-flex flex-wrap justify-content-between align-items-center mb-5">
    <h3 class="fw-bold mb-0">Kanban Cutting</h3>
    <div class="text-muted">Pantau dan kelola proses pemotongan</div>
</div>
...
<div class="row g-7">
    <div class="col-lg-6">
        <div class="card card-flush h-100">
            <div class="card-header">
                <h4 class="card-title mb-0">Menunggu Diproses (Pending)</h4>
            </div>
            <div class="card-body">
                        ...
                        @foreach($pendingOrders as $order)
                            <div class="card border-gray-200 shadow-sm kanban-item"
                                ...
                                <div class="card-footer d-flex justify-content-between align-items-center gap-2 py-3">
                                    ...
                                    <form method="POST" action="{{ route('cutting.start', $order->id) }}" class="ms-auto form-start">
                                        @csrf
    ...
    <div class="col-lg-6">
        <div class="card card-flush h-100">
            <div class="card-header">
                <h4 class="card-title mb-0">Sedang Dikerjakan (In Progress)</h4>
            </div>
            <div class="card-body">
                        ...
                        @foreach($inProgressOrders as $order)
                            <div class="card border-gray-200 shadow-sm kanban-item"
                                ...
                                <div class="card-footer d-flex justify-content-between align-items-center gap-2 py-3">
                                    ...
                                    <form method="POST" action="{{ route('cutting.complete', $order->id) }}" class="ms-auto form-complete">
                                        @csrf
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    selectSort && selectSort.addEventListener('change', function(){
        applySort();
        // keep current search filter after sort
        applySearch();
    });
    ...
    document.querySelectorAll('.form-complete').forEach(function(form){
        form.addEventListener('submit', function(e){
            ...
            if (window.Swal && Swal.fire) {
                e.preventDefault();
                Swal.fire({
                    title: 'Selesaikan Potong?',
                    text: 'Yakin potongan sudah selesai?',
                    icon: 'question',
                ...
                proceed = window.confirm('Yakin potongan sudah selesai?');
                if (!proceed) e.preventDefault();
            }
        });
    });

    // Confirm start process (same pattern as complete)
    document.querySelectorAll('.form-start').forEach(function(form){
        form.addEventListener('submit', function(e){
            ...
            if (window.Swal && Swal.fire) {
                e.preventDefault();
                Swal.fire({
                    title: 'Mulai Proses Cutting?',
                    text: 'Yakin mulai memproses order ini?',
                    icon: 'question',
                    showCancelButton: true,
                ...
                proceed = window.confirm('Yakin mulai memproses order ini?');
                if (!proceed) e.preventDefault();
            }
        });
    });
