...
@section('page_title', 'Riwayat Pemotongan')

@section('content')
<div class="d-flex flex-wrap justify-content-between align-items-center mb-5">
    <h3 class="fw-bold mb-0">Riwayat Pemotongan</h3>
    <a href="{{ route('cutting.dashboard') }}" class="btn btn-light"><i class="bi bi-arrow-left me-1"></i>Kembali ke Kanban</a>
</div>
...
<div class="card card-flush">
    <div class="card-body py-5">
        ...
        <div class="table-responsive">
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="cutting_history_table">
                <thead>
                ...
                <tbody class="fw-semibold text-gray-600">
                    @foreach(($historyOrders ?? collect()) as $i => $order)
                        ...
                        <tr>
                            ...
                            <td>{{ $end ?? '-' }}</td>
                            <td>{{ $dur }}</td>
                            <td><span class="badge bg-success">Selesai Dipotong</span></td>
                            <td class="text-end">
...
<script>
    document.addEventListener('DOMContentLoaded', function(){
        ...
        if (window.jQuery && jQuery.fn && jQuery.fn.DataTable) {
            dt = jQuery('#cutting_history_table').DataTable({
                pageLength: 10,
                ordering: true
            });
        }
